...
def addAnnotation(inputFilePath, outputFilePath, genome_id, is_grc):
    ...
    gene_bed = f"{outputFilePath}.tmp.refGene.bed.gz"
    exon_bed = f"{outputFilePath}.tmp.refExon.bed.gz"

    annot_utils.gene.make_gene_info(gene_bed, "refseq", genome_id, is_grc, False)
    annot_utils.exon.make_exon_info(exon_bed, "refseq", genome_id, is_grc, False)
    gene_tb = pysam.TabixFile(gene_bed)
    exon_tb = pysam.TabixFile(exon_bed)

    print('\t'.join([
        "Chr_1", "Pos_1", "Dir_1", "Chr_2", "Pos_2", "Dir_2", "Inserted_Seq", "Variant_Type",
    ...
    for line in hIN:
        ...
        print('\t'.join[
            '\t'.join(F[0:7]), SVtype, ';'.join(gene1), ';'.join(gene2),
            ';'.join(exon1), ';'.join(exon2), '\t'.join(F[7:])], file=hOUT)
#               '\t'.join(F[7:11]) + '\t' + str(round(tumorAF, 4)) + '\t' + str(round(normalAF, 4)) + '\t' + str(round(float(F[11]), 4))

    subprocess.check_call(["rm", "-rf", gene_bed])
    subprocess.check_call(["rm", "-rf", f"{gene_bed}.tbi"])
    subprocess.check_call(["rm", "-rf", exon_bed])
    subprocess.check_call(["rm", "-rf", f"{exon_bed}.tbi"])
