...
def genomonSV_parse(args):
    ...
    parseFunction.parseJunctionFromBam(args.bam_file, f"{args.output_prefix}.junction.unsort.txt", args.junction_min_mapping_qual,
    ...
    utils.sortBedpe(args.output_prefix + ".junction.unsort.txt", f"{args.output_prefix}.junction.sort.txt")

    utils.processingMessage("getting start positions of paired-end reads")
    parseFunction.getPairStartPos(f"{args.output_prefix}.junction.sort.txt", f"{args.output_prefix}.junction.pairStart.bed")

    utils.processingMessage("compressing start position infomation file")
    utils.compress_index_bed(f"{args.output_prefix}.junction.pairStart.bed", f"{args.output_prefix}.junction.pairStart.bed.gz")


    utils.processingMessage("getting covered regions of paired-end reads from bam file")
    parseFunction.getPairCoverRegionFromBam(args.bam_file, f"{args.output_prefix}.junction.pairCoverage.txt", f"{args.output_prefix}.junction.pairStart.bed.gz")

    utils.processingMessage("adding information of covered regions of paired-end reads")
    parseFunction.addPairCoverRegionFromBam(f"{args.output_prefix}.junction.sort.txt", f"{args.output_prefix}.junction.sort.withPair.txt", f"{args.output_prefix}.junction.pairCoverage.txt")

    utils.processingMessage("clustering breakpoint containing read pairs")
    parseFunction.clusterJunction(f"{args.output_prefix}.junction.sort.withPair.txt", f"{args.output_prefix}.junction.clustered.bedpe.unsort",
    ...
    utils.sortBedpe(f"{args.output_prefix}.junction.clustered.bedpe.unsort", f"{args.output_prefix}.junction.clustered.bedpe")
    ...
    if args.debug == False:
        subprocess.call(["rm", f"{args.output_prefix}.junction.unsort.txt"])
        subprocess.call(["rm", f"{args.output_prefix}.junction.sort.txt"])
        subprocess.call(["rm", f"{args.output_prefix}.junction.pairStart.bed.gz"])
        subprocess.call(["rm", f"{args.output_prefix}.junction.pairStart.bed.gz.tbi"])
        subprocess.call(["rm", f"{args.output_prefix}.junction.sort.withPair.txt"])
        subprocess.call(["rm", f"{args.output_prefix}.junction.pairCoverage.txt"])
        subprocess.call(["rm", f"{args.output_prefix}.junction.clustered.bedpe.unsort"])
        subprocess.call(["rm", f"{args.output_prefix}.junction.pairStart.bed"])
        subprocess.call(["rm", f"{args.output_prefix}.junction.clustered.bedpe"])
    ...
    parseFunction.parseImproperFromBam(args.bam_file, f"{args.output_prefix}.improper.unsort.txt",
    ...
    parseFunction.makeImproperBedpe(f"{args.output_prefix}.improper.unsort.txt", f"{args.output_prefix}.improper.bedpe",
    ...
    parseFunction.clusterImproperBedpe(f"{args.output_prefix}.improper.bedpe", f"{args.output_prefix}.improper.clustered.unsort.bedpe", args.improper_check_margin_size)

    utils.processingMessage("sorting clustered improperly aligned read pairs")
    utils.sortBedpe(f"{args.output_prefix}.improper.clustered.unsort.bedpe", f"{args.output_prefix}.improper.clustered.bedpe")

    utils.processingMessage("compressing clustered improperly aligned read pairs")
    utils.compress_index_bed(f"{args.output_prefix}.improper.clustered.bedpe", f"{args.output_prefix}.improper.clustered.bedpe.gz")

    if args.debug == False:
        subprocess.call(["rm", f"{args.output_prefix}.improper.unsort.txt"])
        subprocess.call(["rm", f"{args.output_prefix}.improper.bedpe"])
        subprocess.call(["rm", f"{args.output_prefix}.improper.clustered.unsort.bedpe"])
        subprocess.call(["rm", f"{args.output_prefix}.improper.clustered.bedpe"])
...
def genomonSV_filt(args):
    ...
    if not os.path.exists(f"{args.output_prefix}.junction.clustered.bedpe.gz"):
        raise ValueError(f'No file: {args.output_prefix}.junction.clustered.bedpe.gz')

    if not os.path.exists(f"{args.output_prefix}.junction.clustered.bedpe.gz.tbi"):
        raise ValueError(f'No file: {args.output_prefix}.junction.clustered.bedpe.gz.tbi')

    if not os.path.exists(f"{args.output_prefix}.improper.clustered.bedpe.gz"):
        raise ValueError(f'No file: {args.output_prefix}.improper.clustered.bedpe.gz')

    if not os.path.exists(f"{args.output_prefix}.improper.clustered.bedpe.gz.tbi"):
        raise ValueError(f'No file: {args.output_prefix}.improper.clustered.bedpe.gz.tbi')

    if args.matched_control_bam != "" and not os.path.exists(args.matched_control_bam):
        raise ValueError(f'No file: {args.matched_control_bam}')

    if args.non_matched_control_junction != "" and not os.path.exists(args.non_matched_control_junction):
        raise ValueError(f'No file: {args.non_matched_control_junction}')

    if not os.path.exists(args.reference_genome):
        raise ValueError(f'No file: {args.reference_genome}')
    ...
    if args.thread_num == 1:
        ...
        for i in range(1, thread_num_mod + 1):
            proc = multiprocessing.Process(target = filterFunction.genomon_sv_filt_main,
                                           args = (f"{args.output_prefix}.thread_{str(i)}", args, f' (thread {str(i)})'))
            jobs.append(proc)
            proc.start()
        ...
        header_flag = 0
        with open(f"{args.output_prefix}.genomonSV.result.txt", 'w') as hout:
            for i in range(1, thread_num_mod + 1):
                with open(f"{args.output_prefix}.thread_{str(i)}.genomonSV.result.txt", 'r') as hin:
        ...
        for i in range(1, thread_num_mod + 1):
            subprocess.check_call(["rm", "-rf", f"{args.output_prefix}.thread_{str(i)}.junction.clustered.bedpe.gz"])
            subprocess.check_call(["rm", "-rf", f"{args.output_prefix}.thread_{str(i)}.junction.clustered.bedpe.gz.tbi"])
            subprocess.check_call(["rm", "-rf", f"{args.output_prefix}.thread_{str(i)}.genomonSV.result.txt"])
...
def genomonSV_merge(args):
    ...
    if os.path.exists(f"{args.merge_output_file}.temp"):
        print(f"Remove existing intermediate file {args.merge_output_file}.temp", file=sys.stderr)
        os.remove(f"{args.merge_output_file}.temp")

    with open(args.control_info_file, 'r') as hin:
        for line in hin:
            label, output_prefix = line.rstrip('\n').split('\t')
            utils.processingMessage(f"extracting information of {label}")
            mergeFunction.simplifyJunc(f"{output_prefix}.junction.clustered.bedpe.gz", f"{args.merge_output_file}.temp", label)

    
    utils.processingMessage("sorting the aggregated junction file")
    utils.sortBedpe(f"{args.merge_output_file}.temp", f"{args.merge_output_file}.temp.sort")

    utils.processingMessage("merging the same junction in the aggregated junction file")
    mergeFunction.organizeControl(f"{args.merge_output_file}.temp.sort", f"{args.merge_output_file}.temp.merged", args.merge_check_margin_size)

    utils.processingMessage("sorting the merged junction file")
    utils.sortBedpe(f"{args.merge_output_file}.temp.merged", f"{args.merge_output_file}.temp.merged.sort")

    utils.processingMessage("compressing the merged junction file")
    utils.compress_index_bed(f"{args.merge_output_file}.temp.merged.sort", args.merge_output_file)


    if args.debug == False:
        subprocess.call(["rm", f"{args.merge_output_file}.temp"])
        subprocess.call(["rm", f"{args.merge_output_file}.temp.sort"])
        subprocess.call(["rm", f"{args.merge_output_file}.temp.merged"])
        subprocess.call(["rm", f"{args.merge_output_file}.temp.merged.sort"])
